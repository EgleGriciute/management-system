...
app.post("/memberships", async (req, res) => {
  ...
  const connection = await mongoClient.connect();
  const data = await connection.db("project").collection("services").insertOne({
    id,
    name,
    price,
    description,
...
app.delete("/memberships/:id", async (req, res) => {
  ...
  const connection = await mongoClient.connect();
  const data = await connection.db("project").collection("services").deleteOne({
    id: membershipId,
  });
  res.send(data);
});
...
app.post("/users", async (req, res) => {
  ...
  const connection = await mongoClient.connect();
  const data = await connection.db("project").collection("users").insertOne({
    name,
    surname,
    email,
    membership,
